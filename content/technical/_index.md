...
description: "Параметры тоннеля, маглев-системы, энергетики, капсулы и гелиевый буфер на выходе (v3.2)"
weight: 25
cascade:
  showAuthor: true
  showDate: true
...
> Версия v3.2 (май 2026). Шлюз вакуум→атмосфера решён через гелиевый буфер последних 2 км + кевларовую мембрану + Fast-Acting Valve. Скорость в тоннеле снижена с 6,5 до 2,5 км/с. См. [главу 12](https://github.com/antonkuzmenkov/uraldriver-site/blob/master/concept-document/chapters/12_v3_2_pivot.md).
...
|---|---|---|---|---|
| T+0 сек | 1700 м н.у.м., начало тоннеля | 0 м/с | Старт LSM-разгона | 8 g продольных |
...
![Drag-spike профиль: ствол маглева в разрезе, перегрузка 8 g на разгоне → 4,5 g spike в гелиевом буфере → плавное затухание; динамическое давление q ÷ 7 за счёт гелия](/img/v3_2/01_drag_spike_profile.png)
...
В стволе перегрузка 8 g 32 секунды — режим пригоден для пилотируемых (космонавтов на центрифуге тренируют до 9 g по минуте). На входе в гелиевый буфер перегрузка падает до нуля (нет тяги). Drag-spike при пробитии кевларовой мембраны — **4,5 g за 0,5 секунды**, не 450 g (это была ошибка v1 с Cd=0,5, реальный Cd острого конуса 0,18). Динамическое давление в гелии в 7 раз ниже атмосферного — гелий тормозит, не разрушает.
...
| Перегрузка длительная на разгоне | 8 g продольных |
