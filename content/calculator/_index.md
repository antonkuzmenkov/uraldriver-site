...
description: "Интерактивная модель удельной стоимости пуска. Подвигай ползунки и посмотри, как меняется $/кг при разных капитальных затратах, темпе пусков и эксплуатационных расходах."
weight: 35
date: 2026-05-11
---
...
<div id="ud-calc">
  ...
  <div class="ud-calc-row">
    <label>эксплуатационные затраты операционные расходы, $ млн/год: <output id="opex-out">370</output></label>
  ...
  <div class="ud-calc-result">
    ...
    <div class="ud-calc-big">
      <span class="ud-calc-label">$/кг УДЕЛЬНАЯ (эксплуатационные затраты/ПН)</span>
...
(function() {
  ...
  function recalc() {
    ...
    if (marginalCostPerKg < 200) {
      verdict = `<strong>Сценарий А — конкурентен многоразовым тяжёлым ракетам.</strong> При ${rate} пусков/год и $${opex}М эксплуатационные затраты удельная стоимость $${fmt(marginalCostPerKg)}/кг пробивает целевую многоразовые тяжелые ракеты $100-200/кг. Это режим максимального государственного спроса (Cislunar economy раскрылся к 2045+).`;
    } else if (marginalCostPerKg < 500) {
      verdict = `<strong>Сценарий Б — базовый v3.2.</strong> Удельная стоимость $${fmt(marginalCostPerKg)}/кг — реалистичный режим для массового топлива и металлов к околунному депо. УД и многоразовые тяжелые ракеты — на разных рынках, не конкуренты.`;
...
**Полная стоимость** = (капитальные затраты/срок_амортизации + эксплуатационные затраты) / (пусков_в_год × ПН_кг).

**Удельная стоимость** = эксплуатационные затраты / (пусков_в_год × ПН_кг). Без учёта капитальных затрат — это «следующий пуск», то что коммерчески важно для конкуренции со многоразовые тяжелые ракеты.

## Базовый сценарий v3.2

- **капитальные затраты**: $28 млрд за 15 лет программы 2030-2045 (тоннель, магниты, ВВЭР, импульсная энергетика, капсулы)
- **эксплуатационные затраты**: $370 млн/год (электричество $150М + персонал $50М + амортизация капсул $80М + расходники + страховка)
...
Базовый сценарий Б даёт удельную **$300-500/кг**. Это **в 3-10 раз дороже** целевой многоразовые тяжелые ракеты $100-200/кг — и в этом не дефект, а другая ниша: УД — стратегическая инфраструктура для bulk-грузов на полярные орбиты, не коммерческая ракета.
